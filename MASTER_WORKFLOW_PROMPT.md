...
### Branch A: Agent-Led Discovery (Rigorous Verification)
**Action:** Follow the protocol defined in `SEARCH_AND_VERIFY_IMAGES.md` strictly.
1.  **Parse:** Convert the shot list into "Search Intent Cards".
2.  **Search:** Prioritize Tier A sources (Wikimedia, Official Boards) over generic searches.
3.  **Verify:** Perform mandatory download verification (HTTP Status 200, Content-Type, Size).
4.  **Manifest:** Generate the highly detailed JSON manifest as specified in the protocol.
5.  **Download:** Use the verified direct URLs to populate `public/images`.
